...
// DetectPlatform returns the running host's Platform. The second return value
// is false on architectures or operating systems the billing API does not yet
// publish assets for.
...
func PickDownloadURL(baseURL, product, channel string, fallback *Platform) (string, bool) {
	...
	return DownloadURL(baseURL, product, channel, platform), true
}
